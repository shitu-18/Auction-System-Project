var ForeSee = {
    ...
    'Date:': '11/07/2008',
    'enabled': true,
    'files': '/includes/foresee/',
    'id': 'lIJBFI8Npx14wll5B5EMlg==',
    'sites': [{
        path: '.',
        cookie: 'cookie',
        domain: 'payless.com'
...
(function(){var D=ForeSee.sites;for(var C=0,A=D.length;C<A;C++){if(document.location.href.match(D[C].path)){ForeSee.siteid=C;
ForeSee.site=ForeSee.sites[ForeSee.siteid];var B=new Date();B.setTime(B.getTime()+(24*60*60*1000));
document.cookie="foresee.talive=true;expires="+B.toGMTString()+";path=/;domain="+ForeSee.site.domain+";";
break}}})();fsr$dbug={log:function(){}};ForeSee.Native=function(J){J=J||{};var F=J.afterImplement||function(){};
...
if(A){var D=A.toLowerCase();C.prototype.fsr$family={name:D};ForeSee.Native.typize(C,D)}var I=function(M,K,N,L){if(!B||L||!M.prototype[K]){M.prototype[K]=N
...
}:function(A){return A.fsr$uid||(A.fsr$uid=ForeSee.Native.UID++)};ForeSee.Window=new ForeSee.Native({name:"Window",legacy:(ForeSee.Browser.Engine.trident)?null:window.Window,initialize:function(A){ForeSee.$uid(A);
...
}});ForeSee.Window.Prototype={fsr$family:{name:"window"}};new ForeSee.Window(window);ForeSee.Document=new ForeSee.Native({name:"Document",legacy:(ForeSee.Browser.Engine.trident)?null:window.Document,initialize:function(A){ForeSee.$uid(A);
...
for(var B=0,A=C.length;B<A;B++){ForeSee.Browser.freeMem(C[B])}}});ForeSee.Element=new ForeSee.Native({name:"Element",legacy:window.Element,initialize:function(A,B){var C=ForeSee.Element.Constructors.fsr$get(A);
...
},fsr$toQueryString:function(){var G=ForeSee.c();var D=G.fsr$get("browser");var F={browser:D.name+" "+D.version+" on "+D.platform,url:G.fsr$get("current"),pv:G.fsr$get("pv"),locale:G.fsr$get("locale")||""};
...
}}}};ForeSee.close=function(){var A=window.open("","fsrTracker");if(A){A.close()}};ForeSee.link=function(A){var C=A;
var B=0;$$fsr("a").fsr$each(function(F){for(var E=0,D=C.length;E<D;E++){if(F.href.match(C)){B++;F.fsr$addEvents({click:function(){ForeSee.controller.popLink()
}});break}}});fsr$dbug.log("linked: "+B)};ForeSee.SurveyController=new ForeSee.Class({Implements:ForeSee.Options,options:{},initialize:function(A){this.setOptions(A);
...
}).fsr$delay(1);return true}if(this.sd.type=="current"){this.link()}return false},processTracker:function(){if(!this.shouldTrack()){return false
...
}if(!(this.sd.sv<=this.sd.sp)){return false}return true},inviteShown:function(){if(ForeSee.$defined(ForeSee.c().fsr$get("invite"))){return true
...
},onFailure:function(){}}).ping()},link:function(){if(this.inviteAccepted()&&!this.surveyShown()){ForeSee.link(this.sd.pop.links||[])
}},init:function(){fsr$dbug.log("==============");var V=ForeSee.c();var B=V.fsr$get("alive")||0;B=B+1;
V.fsr$set("alive",B);this.ralive=true;V=ForeSee.c();B=V.fsr$get("alive");if(!B){fsr$dbug.log("Exit...cookies are not enabled.");
return false}fsr$dbug.log("alive: "+B);ForeSee.Cookie.dispose("foresee.talive",{path:"/",domain:ForeSee.site.domain||false});
fsr$dbug.log("Clear temporary alive cookie.");var W=ForeSee.Browser;V.fsr$set("browser",{name:ForeSee.Browser.Type.name,version:ForeSee.Browser.Type.version,platform:ForeSee.Browser.Platform.os});
fsr$dbug.log("browser: "+W.Type.name+" "+W.Type.version+" on "+W.Platform.os);if(ForeSee.UnsupportedBrowsers[W.Type.name]){if(W.Type.version<=ForeSee.UnsupportedBrowsers[W.Type.name]){fsr$dbug.log("Browser not surpported.");
return false}}var F;if(ForeSee.$defined(V.fsr$get("finish"))){var T=V.fsr$get("timeout");var C=((ForeSee.$time()-V.fsr$get("finish"))/1000);
fsr$dbug.log("ptimeout: "+T);fsr$dbug.log("loadtime: "+C);F=(0.9*T)+(0.1*(C*2));if(F<2){F=2}else{if(F>ForeSee.$P().tracker.timeout){F=ForeSee.$P().tracker.timeout
}}}else{F=ForeSee.$P().tracker.timeout}V.fsr$set("timeout",F);fsr$dbug.log("timeout: "+F);var K=V.fsr$get("pv")?V.fsr$get("pv")+1:1;
V.fsr$set("pv",K);fsr$dbug.log("pv: "+K);if(!V.fsr$get("start")){var M;if(M=ForeSee.Cookie.read("foresee."+ForeSee.site.cookie+".repeatdays",{path:"/",domain:ForeSee.site.domain||false})){fsr$dbug.log("Persistent Cookie Found: "+M);
return false}V.fsr$set("start",ForeSee.$time());var I=this;this.dhtml_win=1;new ForeSee.Asset.javascript(ForeSee.files+"foresee-dhtml-popup.js",{id:"foresee-dhtml-popup",onload:function(){fsr$dbug.log("DHTML popup script loaded (1).");
I.dhtml_win=2}});this.dhtml_css=1;new ForeSee.Asset.css(ForeSee.files+this.css());this.dhtml_css=2;
this.generateid()}fsr$dbug.log("invite: "+(ForeSee.$pick(V.fsr$get("invite"),"")));fsr$dbug.log("tracker: "+(ForeSee.$pick(V.fsr$get("tracker")||"")));
ForeSee.sv=ForeSee.$random(1,100);this.sp=new ForeSee.Hash.Cookie("foresee.sp",{path:"/",domain:false});
var D,E,H,N,S,R;N=V.fsr$get("current");R=V.fsr$get("cdi");D=document.location.href;V.fsr$set("current",D);
E=this.match(D);if(E.length!=0){fsr$dbug.log("===CURRENT====");var O=V.fsr$get("lc")||{};var U=V.fsr$get("ec")||{};
for(var Q=0,P=E.length;Q<P;Q++){var A=ForeSee.surveydefs[E[Q]];A.idx=E[Q];A.lc=O[A.name]=O[A.name]?O[A.name]+1:1;
A.ec=U[A.name]=U[A.name]?U[A.name]:0;A.type="current";this.configLoyalty(A);var L=this.loyaltyDef(A);
var G=this.checkLoyalty(L,A.lc,A.ec);if(G>-1){A.ls=true;if(ForeSee.$type(A.lf)=="array"){A.lf=A.lf[G];
A.sp=A.sp[G];A.pop.when=A.pop.when[G]}if(A.pin){var J=V.fsr$get("p")||{};J[A.name]=1;V.fsr$set("p",J)
}}else{A.ls=false;if(ForeSee.$type(A.lf)=="array"){A.lf=A.lf[0];A.sp=A.sp[0];A.pop.when=A.pop.when[0]
}}this.configure(A);H=A.idx;V.fsr$set("cdi",A.idx);this.cd=A;break}V.fsr$set("lc",O);V.fsr$set("ec",U)
}if(R&&(R!=H)){fsr$dbug.log("===PREVIOUS===");var A=ForeSee.surveydefs[R];A.idx=R;A.lc=V.fsr$get("lc")[A.name];
A.type="previous";this.configLoyalty(A);A.ls=true;this.configure(A);this.pd=A}fsr$dbug.log("==============");
if(!this.cd&&!this.pd){return false}return true},configLoyalty:function(A){if(ForeSee.$type(A.lf)=="number"){A.lf={v:A.lf,o:">="}
}},loyaltyDef:function(B){var A=B.lf;if(ForeSee.$type(B.lf)=="object"){A=[B.lf]}return A},checkLoyalty:function(E,F,C){var B=-1;
for(var D=0,A=E.length;D<A;D++){if(E[D].o==">="){if(F>=E[D].v){B=D}}else{if(E[D].o=="="){if((F-C)==E[D].v){B=D
}}else{if(E[D].o==">"){if(F>E[D].v){B=1}}}}}return B},configure:function(B){var D=ForeSee.c();fsr$dbug.log("sid: "+B.name);
fsr$dbug.log("lc: "+B.lc);fsr$dbug.log("lf: "+B.lf.v+" ("+B.lf.o+") ");B.sv=ForeSee.sv;fsr$dbug.log("sv: "+B.sv);
if(ForeSee.$type(B.sp)=="array"){B.sp=B.sp[(new Date()).getDay()]}B.sp=this.sp.fsr$get(B.name)||B.sp;
fsr$dbug.log("sp: "+B.sp);if(B.invite){B.invite=ForeSee.$merge(ForeSee.$P().invite,B.invite)}B.tracker=ForeSee.$merge(ForeSee.$P().tracker,B.tracker);
B.survey=ForeSee.$merge(ForeSee.$P().survey,B.survey);B.qualifier=ForeSee.$merge(ForeSee.$P().qualifier,B.qualifier);
B.pop=ForeSee.$merge(ForeSee.$P().pop,B.pop);B.repeatdays=ForeSee.$pick(ForeSee.$P().repeatdays,B.repeatdays);
var A=[].fsr$extend(ForeSee.$P().exclude.local);if(B.exclude&&B.exclude.local){B.exclude.local=A.fsr$extend(B.exclude.local)
}else{if(B.exclude){B.exclude.local=A}else{B.exclude={local:A}}}var C=[].fsr$extend(ForeSee.$P().exclude.referer);
if(B.exclude&&B.exclude.referer){B.exclude.referer=C.fsr$extend(B.exclude.referer)}else{if(B.exclude){B.exclude.referer=C
}else{B.exclude={referer:C}}}},unload:function(){if(!ForeSee.enabled){return }if(!this.runload&&this.ralive){this.runload=true;
this.uninit()}return },uninit:function(){var B=ForeSee.c();var A=B.fsr$get("alive")||0;B.fsr$set("alive",(A-1)>0?A-1:0);
B.fsr$set("previous",B.fsr$get("current"));B.fsr$set("finish",ForeSee.$time());ForeSee.Cookie.dispose("foresee.talive",{path:"/",domain:ForeSee.site.domain||false})
},match:function(A){if(!A){return }var E=[];var K=ForeSee.surveydefs;var B=ForeSee.c().fsr$get("p")||{};
for(var J=0,C,F=K.length,G=0;J<F;J++){C=K[J].include.urls||[];var I=G;for(var H=0,D=C.length;H<D;
H++){if(A.match(C[H])){E[G++]=J;break}}if(G!=I){break}C=K[J].include.cookies||[];for(var H=0,D=C.length;
H<D;H++){var L;if(L=ForeSee.Cookie.read(C[H].name,{path:C[H].path||false,domain:C[H].domain||false})){if(L.match(C[H].value||".")){E[G++]=J;
break}}}if(G!=I){break}if(B[K[J].name]){E[G++]=J}if(G!=I){break}}return E},invite:function(){this.language();
...
}).fsr$delay(1)}}}},showInvite:function(){var B=this;var A=this.sd.invite;this.page(A);var H=document.location.protocol;
var F=document.location.host;var G=document.location.path||"";var D=ForeSee.files+A.url;var C=H+"//"+F+G+D;
var E=new ForeSee.StickyWinModal.Ajax({url:C,position:{x:A.x,y:A.y},wrapWithUi:true,uiOptions:{width:A.width+"px",baseHref:ForeSee.files,buttons:[{properties:{id:"accept"},text:A.accept,onClick:function(){B.accepted()
...
E.fsr$addEvent("onClose",function(){B.close()});ForeSee.invite=0;E.update()},accepted:function(){ForeSee.invite=1;
ForeSee.log("Invite Accepted");ForeSee.c().fsr$set("invite",ForeSee.invite);if(ForeSee.$P().mode=="hybrid"){new ForeSee.Service({host:ForeSee.services.domain.host,path:ForeSee.services.domain.path,url:ForeSee.services.domain.url,params:{"do":1,rw:this.sd.repeatdays*24*60}}).ping()
...
},close:function(){ForeSee.c().fsr$set("invite",ForeSee.invite);if(this.sd.repeatdays){ForeSee.Cookie.write("foresee."+ForeSee.site.cookie+".repeatdays",this.sd.repeatdays,{path:"/",domain:ForeSee.site.domain||false,duration:this.sd.repeatdays})
}},popAccept:function(){if(this.sd.pop.when=="later"){if(this.sd.pop.tracker){this.popTracker()}if(this.sd.pop.links){this.link()
}}else{if(this.sd.pop.when=="now"){ForeSee.c().fsr$set("survey",1);if(!this.sd.pop.what!="qualifier"){this.popSurvey()
...
}else{this.popQualifier()}}},popSurvey:function(){var G=ForeSee.services.survey;var H=this.sd.survey;
var K=this.sd.pop;var C=(window.screen.width-H.width)/2;var J=(window.screen.height-H.height)/2;var L=document.location.protocol;
var M=G.host;var N=G.path;var I=ForeSee.c();var E=new ForeSee.Hash({sid:this.sid(),cid:ForeSee.id,version:ForeSee.version}).fsr$toQueryString();
var F=ForeSee.CPPS.fsr$toQueryString();var A=L+"//"+M+N+G.url+"?"+E+"&"+F;if(H.loading){ForeSee.Cookie.write("foresee.url",A,{path:"/",domain:ForeSee.site.domain||false});
this.page(ForeSee.$P().loading);var D=document.location;var B=ForeSee.files+ForeSee.$P().loading.url;
A=L+"//"+D.host+(D.path||"")+B+"?domain="+ForeSee.site.domain}this.pop("fsrSurvey",A,C,J,H.width,H.height,K.pu);
ForeSee.log("Survey Shown",ForeSee.c().fsr$get("current"))},popTracker:function(){var E=this.sd.tracker;
var F=this.sd.pop;this.page(E);var B=(window.screen.width-E.width)/2;var D=(window.screen.height-E.height)/2;
var G=document.location.protocol;var H=document.location.host;var I=document.location.path||"";var C=ForeSee.files+E.url;
var A=G+"//"+H+I+C+"?siteid="+ForeSee.siteid+"&files="+ForeSee.files;this.pop("fsrTracker",A,B,D,E.width,E.height,true);
ForeSee.log("Tracker Shown",ForeSee.c().fsr$get("current"))},popQualifier:function(){var F=ForeSee.services.survey;
var G=this.sd.qualifier;var J=this.sd.pop;this.page(G);var C=(window.screen.width-G.width)/2;var I=(window.screen.height-G.height)/2;
var K=document.location.protocol;var L=G.location=="local"?document.location.host:F.host;var M=G.location=="local"?document.location.path||"":F.path;
var B=ForeSee.files+G.url;var H=ForeSee.c();var D=new ForeSee.Hash({version:ForeSee.version}).fsr$toQueryString();
var E=ForeSee.CPPS.fsr$toQueryString();var A=K+"//"+L+M+B+"?"+D+"&"+E;this.pop("fsrQualifier",A,C,I,G.width,G.height,J.pu);
ForeSee.log("Qualifier Shown",ForeSee.c().fsr$get("current"))},popLink:function(){if(!this.surveyShown()){this.popImmediate()
}},sid:function(){var D=ForeSee.c();var B=this.sd.name;var C=this.sd.pop.now;if(C){B=B+"-"+C}var A=D.fsr$get("locale");
if(A){B=B+"-"+A}return B},pop:function(D,C,H,G,F,A,E,B){(function(){new ForeSee.Browser.Popup(C,{name:D,toolbar:0,location:0,directories:0,status:0,scrollbars:1,resizable:1,width:F,height:A,x:H,y:G,blur:E,menubar:0})
}).fsr$delay(B||0)},language:function(){if(!ForeSee.$P().language){return }var E=ForeSee.$P().language;
var D;if(E.src=="location"){D=document.location.href}else{if(E.src=="cookie"){if(E.type&&E.type=="client"){D=ForeSee.Cookie.read(E.name,{path:"/",domain:ForeSee.site.domain||false})
}else{D=ForeSee.c().fsr$get("locale")}}}var B=E.locales||[];for(var C=0,A=B.length;C<A;C++){if(D.match(B[C].match)){ForeSee.c().fsr$set("locale",B[C].locale);
break}}},page:function(E){var B=ForeSee.c().fsr$get("locale");if(!B){return }var D=E.pages||[];for(var C=0,A=D.length;
C<A;C++){if(D[C].locale==B){E.url=D[C].url;if(D[C].accept){E.accept=D[C].accept}if(D[C].decline){E.decline=D[C].decline
}if(D[C].width){E.width=D[C].width}if(D[C].height){E.height=D[C].height}break}}},generateid:function(){if(!ForeSee.services.event.enabled){return 
...
}return B}});new ForeSee.SurveyController({});window.fsr$addEvent("domready",function(){(function(){ForeSee.controller.load()
}).fsr$delay(1)});window.fsr$addEvent("unload",function(){ForeSee.controller.unload()});